const insertDummyData = (db, transactions_db) => {
    ...
    for(var i = 0; i<=9; i+=1)
    {
        ...
        db.query(`insert into EventManager (eventManagerEmail,name,password,timeStamp,managerPhoneNumber) values ('managerEmail${i}','managerName${i}','managerPass${i}','${date_time}','123456789${i}')`); 
        db.query(`insert into EventData (eventName,eventOrWorkshop,departmentAbbr,description,eventManagerEmail,date,eventTime,venue,fees,totalNumberOfSeats,noOfRegistrations,timeStamp,refundable) values ('EVENT${i}','${i%2}','DEP${i}','DESCRIPTION${i}','managerEmail${i}', '2${i}-Mar-2023', '${i}0:00:00','VENUE${i}',${(i+1)*100},${i+50},${i+10},'${date_time}',${bool_data})`);
