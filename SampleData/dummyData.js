const insertDummyData = (db, transactions_db) => {
    ...
    for(var i = 0; i<=9; i+=1)
    {
       ...
       transactions_db.query(`insert into Transactions (transactionId,productId, userEmail,senderName, eventIdOrPassportId,amount,timeStamp, transactionStatus, address ,city, state , zipcode, phoneNumber ) values ('ANK738278372873${i}', 'PID${i}', 'cb.en.u4cse2001${i}@cb.students.amrita.edu', 'SENDER${i}', 0, 100,'${date_time}', 'INITIATED', 'ADDRESS${i}', 'CITY${i}', 'STATE${i}', '98765432${i}', '666634${i}')`);
